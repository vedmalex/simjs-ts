const QueueApp = {
	...
	reset(dontask) {
		...
		const l1 = this.canvas.path(a.join(""));
		l1.attr({ "stroke-width": 0.5, stroke: "pink" });
		this.canvas.path("M0 0L0 400L600 400L600 0L0 0").attr({ "stroke-width": 4, stroke: "pink" });

		for (var i = 0; i < 4; i++) {
			this.canvas.rect(10, 10 + 50 * i, 50, 50).attr({ fill: "#FAF6AA", "fill-opacity": "50", stroke: "F7D68A" });
		}

		const t = this.parseTime(this.until);
		$("#config_sim").button("option", "label", `${t[0].toFixed(3)} ${t[1]}`);

		const q = this.canvas.image("images/server.png", 12, 25, 46.4, 22);
		const s = this.canvas.image("images/customers.png", 15, 70, 34, 34);
		const sp = this.canvas.image("images/splitter.png", 15, 115, 41 * 0.9, 48 * 0.9);
		const si = this.canvas.image("images/door_out.png", 18, 165, 32, 32);
	...
	newView(ViewFn, ModelFn, type, name, hasIn, hasOut, x, y) {
		if (!x) {
			x = this.posx;
			y = this.posy;
			this.updateDrop();
		}
		const obj = new ViewFn(this.canvas, type, name, x, y, hasIn, hasOut);
		if (this.showConn) obj.showDots(true);
		this.views.push(obj);

		const model = new ModelFn(obj);
		obj.model = model;
	...
	newServer(x, y) {
		this.server_id++;
		return this.newView(ImageView, ServerModel, "queue", `queue_${this.server_id}`, true, true, x, y);
	},

	newSource(x, y) {
		this.source_id++;
		return this.newView(ImageView, SourceModel, "source", `source_${this.source_id}`, false, true, x, y);
	},

	newSink(x, y) {
		this.sink_id++;
		return this.newView(ImageView, SinkModel, "sink", `sink_${this.sink_id}`, true, false, x, y);
	},

	newSplitter(x, y) {
		this.splitter_id++;
		return this.newView(SplitterView, SplitterModel, "splitter", `splitter_${this.splitter_id}`, true, true, x, y);
	},
	...
	saveSettings() {
		...
		const mult = d.find("#time_selector").val();
	...
	run() {
		...
		const completed = app.sim.simulate(app.until, app.EventsPerInterval);
	...
	complete() {
		...
		for (let i = QueueApp.models.length - 1; i >= 0; i--) {
			const model = QueueApp.models[i];
	...
	if (line && line.line) {
		...
		return {
			bg:
				bg &&
				bg.split &&
				this.path(path).attr({ stroke: bg.split("|")[0], fill: "none", "stroke-width": bg.split("|")[1] || 3 }),
