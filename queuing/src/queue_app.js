const QueueApp = {
  init() {
    ...
    // Simulation menu
    $("#play_sim").button({icons: {primary: "ui-icon-play"}}).click(() => {
      QueueApp.startSim();
    });
    $("#config_sim").button({icons: {primary: "ui-icon-clock"}}).click(() => {
      QueueApp.showSimProperties();
    });
    $("#sim_ops").buttonset();

    // buttons that are shown when simulation is running
    $("#pause_sim").button({text: false, icons: {primary: "ui-icon-pause"}}).click(() => {
      if (QueueApp.paused) {
    ...
    $("#stop_sim").button({text: false, icons: {primary: "ui-icon-stop"}}).click(() => {
...
Raphael.fn.connection = function (obj1, obj2, line, bg) {
   ...
   const bb1 = obj1.getBBox(), bb2 = obj2.getBBox(), x1 = bb1.x + bb1.width + 1, y1 = bb1.y + bb1.height / 2, x4 = bb2.x - 1, y4 = bb2.y + bb2.height / 2, res = [3, 6];

    dx = Math.max(Math.abs(x1 - x4) / 2, 10);
    dy = Math.max(Math.abs(y1 - y4) / 2, 10);
    const x2 = [x1, x1, x1 - dx, x1 + dx][res[0]].toFixed(3), y2 = [y1 - dy, y1 + dy, y1, y1][res[0]].toFixed(3), x3 = [0, 0, 0, 0, x4, x4, x4 - dx, x4 + dx][res[1]].toFixed(3), y3 = [0, 0, 0, 0, y1 + dy, y1 - dy, y4, y4][res[1]].toFixed(3);
