...
class ServerEntity extends Entity {
	start(random, nservers, mu) {
		this.random = random;
		this.mu = mu;
		this.facility = CreateFacility("queue");
	}
...
/*-------------------------*/
class SplitterEntity extends Entity {
	start(random, prob) {
		this.random = random;
		this.prob = prob;
	}
...
function QueueSimulator(jsontext) {
	...
	const dict = {};
	const ModelFactory = { queue: ServerModel, source: SourceModel, splitter: SplitterModel, sink: SinkModel };
