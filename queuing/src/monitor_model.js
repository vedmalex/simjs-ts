class MonitorModel {
	constructor(view) {
		...
		this.stat = [this.statTable.find("#arrival"), this.statTable.find("#inter"), this.statTable.find("#interd")];
	}
	...
	showSettings(x, y) {
		...
		d.dialog("option", { title: this.view.name, position: [x, y] }).dialog("open");
	}
